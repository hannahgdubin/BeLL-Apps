...
class MemberTest(BaseCase):
    ...
    def test_add_member(self):
        ...
        # random string to fill all fields
        fill = "a"
        ...
        self.setup()
        # fill up the form without uploading a file
        self.fill_form(fill, data, False)
        #TODO: test new member login
...
#        self.fill_form(fill, data, True, True, attachment)
    ...
    def fill_form(self, fill, data, new=True, upload=False, attachment=None):
        ...
        self.assertEqual(actual, expected)
        Alert(driver).accept()
        # if the user already exists, then add a new user
        if not new:
            fill = "".join(choice(ascii_lowercase) for i in range(3))
            self.fill_form(fill, data, True)
